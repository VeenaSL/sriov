...
from nova.objects import pci_device as pd
...
class PciDevTracker(object):
    ...
    def __init__(self, node_id=None):
        ...
        if node_id:
            self.pci_devs = list(
                pd.PciDeviceList.get_by_compute_node(context, node_id))
        else:
            self.pci_devs = []
    ...
    def set_hvdevs(self, devices):
        ...
        for dev in [dev for dev in devices if
            ...
            dev_obj = pd.PciDevice.create(dev)
            self.pci_devs.append(dev_obj)
            self.stats.add_device(dev_obj)

    def _claim_instance(self, context, instance, prefix=''):
        pci_requests = objects.instance_pci_requests.InstancePCIRequests.get_by_instance(
            context, instance)
...
def get_instance_pci_devs(inst, request_id=None):
    ...
    if isinstance(inst, instance.Instance):
        ...
        pci_devices = pd.PciDeviceList.get_by_instance_uuid(
            ctxt, inst['uuid'])
