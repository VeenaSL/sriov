...
class PciDevTrackerTestCase(test.TestCase):
    ...
    def _create_pci_requests_object(self, mock_get, requests):
        ...
        for request in requests:
            pci_req_obj = objects.instance_pci_requests.InstancePCIRequest(count=request['count'],
                                                     spec=request['spec'])
            pci_reqs.append(pci_req_obj)
        mock_get.return_value = objects.instance_pci_requests.InstancePCIRequests(requests=pci_reqs)
